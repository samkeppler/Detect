...
def run(subject, df_data, df_demog, regress, tracts, hemi, metric, model_type='AutoEncoder'):
    ...
    if model_type == "ZScore":
        ...
        if x_hat is None or np.isnan(x_hat).any():
            st.error("AutoEncoder produced NaNs or failed to reconstruct.")
            return None, None, None, None, None, 0

        x_hat_inv = scaler.inverse_transform(x_hat)
        x_inv = scaler.inverse_transform(X_test)

        if np.isnan(x_hat_inv).any() or np.isnan(x_inv).any():
            st.error("NaNs detected in input or reconstructed data.")
            return None, None, None, None, None, 0
